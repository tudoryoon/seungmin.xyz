...
`functions/api/substack.js` fetches only `https://tudoryoon.substack.com/feed`, without forwarding visitor cookies or authorization. It has an 8-second upstream timeout, a 3 MiB limit, and a five-minute public edge/browser cache; failures are not cached. `_routes.json` invokes the Function only for `/api/substack` and its trailing-slash variant. All other pages and assets stay static. No secret, database, private Substack account access, or paid service is configured. Pages Functions usage follows the account's existing Cloudflare plan.

The browser fetches lazily on opening Substack, parses RSS with native DOMParser, displays up to 30 recent public feed entries, and reuses results for one minute. While the reader is visible it checks every five minutes, and also checks when focus or visibility returns. Other pages and background tabs do not fetch. Refresh preserves scroll, keyboard focus and the currently open article. Titles/authors use text nodes; article HTML is sanitized with vendored DOMPurify 3.4.15 and restricted to reading markup. Scripts, embeds, forms, style attributes and unsafe URLs are removed. Images are restricted to the publication and Substack CDN; links use noopener/noreferrer. Only public RSS content is shown; a paid or truncated post stays truncated, with an explicit original-post link. Empty feeds show an empty state; errors have a retry control. New posts appear after the RSS/cache refresh, without code changes. No signup, subscription purchase or user-data import is performed.
