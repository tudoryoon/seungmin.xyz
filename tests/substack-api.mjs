...
try{
  ...
  for(mode of ['throw','redirect','html','large','doctype']){stored=null;const failure=await run();assert.equal(failure.status,502);assert.equal(await failure.text(),'Feed unavailable');assert.equal(failure.headers.get('Cache-Control'),'no-store');}
