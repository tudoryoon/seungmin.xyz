...
Arrow keys move the character at a normalized speed, including diagonal input. Walk to an entrance, then press Enter. Dungeon markers are not clickable and cannot trigger automatic travel. Touch devices show a directional pad and a nearby entrance button. Leaving the map, blur, key release, and logout cancel held-key movement. Ambient motion preferences do not disable deliberate keyboard movement.
