...
The entrance cue uses the existing arrow-down icon, a 48px hit target and a 1.8-second fade/downward motion at the bottom center. Scrolling fades the cue out; reduced-motion settings leave a static arrow. Its button continues the existing entrance sequence. The public entrance hides only the native root scrollbar; wheel, touch, keyboard and reverse scrolling remain native. Policy links live inside About rather than a fixed footer on entry, menu or login screens.

The public `#food-map` tab (서울 맛집 지도) shows all 448 roulette station complexes as a schematic subway map with 508 connections across ten passenger lines. Locally vendored Leaflet handles pan/pinch and fractional zoom; the existing Three.js runtime adds subtle raised tracks, with a complete SVG fallback. There are no geographic map tiles or third-party map requests. It provides station search, line filters, Seoul/all extents and selectable markers; station labels avoid overlap as the view changes. Data, geometry generation and tests are documented in `data/food-map.md`. Restaurant records, Supabase and roulette history are unchanged. Desktop uses a station sidebar; mobile places it below the map. The map loads lazily and the depth renderer only draws when the view changes, pausing on hidden tabs.
