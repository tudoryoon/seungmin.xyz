...
The public `#food-map` tab (서울 맛집 지도) maps all 448 roulette station complexes, using real coordinates, a locally vendored Leaflet engine and OpenStreetMap tiles. It provides station search, ten passenger-line filters, Seoul/all extents, zoom and selectable markers with station details. Map data, licensing, tile policy and tests are documented in `data/food-map.md`. This release has no restaurant records or write controls; Supabase and roulette history are unchanged. Desktop uses a station sidebar, mobile places it below the map. Map code/tiles initialize only when the tab is visible, and list-based station selection remains available if the map cannot load.
