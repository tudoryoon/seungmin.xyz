...
The native page scroll drives a 620svh entrance through a point globe, braided helical trails and a layered particle spiral. Its 520svh scrollable runway is twice the previous length. Three overlapping eased camera legs keep forward and reverse travel continuous. Near particles fade as they pass the camera; distant particles recede, with a slow light pulse along each helix. The point budget is 25,500 on mobile and 48,200 on desktop, with nine fine filaments and no post-processing bloom. These are full-bleed Three.js points and lines, without buildings or enlarged city photos. The login form appears at `#auth`, with the runway still mounted: scrolling up reverses the entire approach without clearing the form. The arrow is a keyboard-accessible shortcut through the same scroll path. Reduced motion disables camera travel and continuous animation. Root overscroll is contained while browser pinch zoom remains available; the background covers the large mobile viewport. Reached login forms stay reached when the viewport changes. Existing signed-in sessions still skip the entrance and restore their map or requested screen. Point-cloud attribution is in [assets/entry-particles.md](assets/entry-particles.md). Existing illustrated scenes remain a no-WebGL fallback; their prompts are in [assets/entry-art-prompts.md](assets/entry-art-prompts.md).
...
Cloudflare's `_headers` requests `no-cache` revalidation. This works on `pages.dev`, but the custom domain currently rewrites static asset responses to `max-age=14400` and removes `no-cache`. Set its Browser Cache TTL to **Respect Existing Headers** in the dashboard when account access is available. Until then, the release query `v=20260918-2` on app CSS/JS URLs and module imports ensures returning visitors fetch the correct release. Bump it consistently across HTML and imports for every code release; headers alone are not sufficient on this domain. No storage clearing or data migration is needed. See [Pages response headers](https://developers.cloudflare.com/pages/configuration/headers/) and [Browser Cache TTL](https://developers.cloudflare.com/cache/how-to/edge-browser-cache-ttl/set-browser-ttl/).
