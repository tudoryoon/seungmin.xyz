...
Scroll-through portal -> public Substack / 개인일정 / About menu. Only 개인일정 opens Supabase email login -> profile -> text-driven pixel avatar -> dungeon map; complete signed-in accounts go directly to their map. Substack currently displays 준비 중 with no external integration. About displays exactly 개인 페이지입니다. Public content opens beneath the horizontal navigation; selecting its active link collapses it. Login retains the navigation, and a back-arrow control returns private screens to the public menu.
